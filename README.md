...
- **Two rules keep Shortcuts' type coercion out of the store.** Shortcuts fills a string parameter with an image's file name ("Clipboard 11 Sep 2026 at 2.33 pm", often with no extension), so (1) whenever Shortcuts hands over an image, the string is ignored outright — deterministic, and the reason the recipe always passes Get Images from Input; (2) `ShortcutsCoercion.textToSave` additionally rejects generated-looking names for shortcuts that don't. **Do not judge this from `UIPasteboard`'s `has*` flags:** they are not dependable from an intent running in the background, which is exactly when this code runs (that mistake shipped and let junk clips through on 11 Sep 2026).
- **Keep generated recipes linear.** Hand-built `WFControlFlowMode` If blocks stopped the picker card from ever presenting on device (11 Sep 2026). An empty variable wired into a file parameter is fine.
