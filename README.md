...
- **Capture and retrieval go through Shortcuts.** One shortcut covers text, links and images: *Get Clipboard → Pick a Clip (Save First: Clipboard, Copied Image: Clipboard) → Copy to Clipboard*, bundled signed in `Klypst/Resources/Shortcuts` and assignable to the Action Button or Back Tap. Shortcuts (privileged) reads and writes the pasteboard; `PickClipIntent` saves, shows the branded Klypst card via `requestConfirmation(snippetIntent:)` (row taps run `SelectPickerClipIntent` and reload the card; the system Copy button returns the selection), and returns an `IntentFile`. Its Style parameter can switch to the plain one-tap system list (`requestDisambiguation`). A snippet can't finish on a row tap, which is why the card needs Copy (the confirmation button is relabelled with `ConfirmationActionName.custom`; its color and placement are the system's). `.glassEffect` inside a snippet makes the Shortcuts host render the card blank (verified on device, iOS 26), so the card uses plain fills only. The snippet route (*Save to Klypst → Recent Clips*) remains as an alternative but must open the app to copy (see spike C).
- **The picker returns a file, not text.** A value that must sometimes carry an image can't be a `String`, so `ClipFile` wraps text and links as a `.utf8PlainText` file and images as their own type; Copy to Clipboard pastes each correctly. `PickImageClipIntent` and `GetClipImageIntent` remain for an images-only trigger.
- **Two rules keep Shortcuts' type coercion out of the store.** Shortcuts fills a string parameter with an image's file name ("Clipboard 11 Sep 2026 at 2.33 pm", often with no extension), so (1) whenever Shortcuts hands over a decodable image in the file parameter, the string is ignored outright — deterministic, and the reason the recipe passes the Clipboard variable into both parameters; (2) `ShortcutsCoercion.textToSave` additionally rejects generated-looking names for shortcuts that don't. **Do not judge this from `UIPasteboard`'s `has*` flags:** they are not dependable from an intent running in the background, which is exactly when this code runs (that mistake shipped and let junk clips through on 11 Sep 2026).
- **A file parameter fed from the clipboard must accept images, plain text and URLs.** Measured with Shortcuts on macOS 26 (same engine as iOS, 11 Sep 2026): an image-only `IntentFile` parameter makes Shortcuts *render copied text into a PNG* to satisfy it, and Get Images from Input does the same, which is what killed the picker card twice (Klypst was handed a picture of the text on every text copy). A parameter that omits `.url` makes Shortcuts *download* a copied link. With all three accepted, text arrives as a tiny text file, an image as itself, and a link as nothing; Klypst then uses the bytes only when they decode as an image. Never add Get Images from Input or If blocks to the generated recipes; keep them linear. The probe app and shortcuts used for these measurements are described in the commit that introduced this rule.
- **Setup is a persisted checklist, not a tour.** Onboarding is four screens: promise, trigger choice, checklist, privacy. The checklist (`SetupChecklistView`) has three steps: add the shortcut (share sheet), assign it to the chosen trigger (steps per trigger in `TriggerGuide`), and run it once. The last step ticks itself: the picker intents run inside the app process and call `AppEnvironment.recordShortcutRun()`, which is the only proof that the shortcut *and* its trigger work. Until then History shows a "Finish setting up" card (dismissable), the empty state offers "Finish setup", and Settings shows the progress. The trigger is only the system setting that runs the one shortcut: Action Button, Back Tap, Control Center, Lock Screen button, Siri or a Home Screen icon (`CaptureTrigger`). Help lists all of them for people whose Action Button is taken. `DeviceCapabilities.hasActionButton` reads the model identifier (no API exists) only to decide which trigger to recommend. "Run it from here" uses the documented `shortcuts://run-shortcut?name=Klypst` scheme after copying a sample to the clipboard.
