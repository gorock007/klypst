...
- **Images go through Shortcuts as files.** `PickImageClipIntent` and `GetClipImageIntent` return an `IntentFile`, so *Pick an Image Clip → Copy to Clipboard* pastes an image without opening the app. `PickClipIntent` stays text-only so existing recipes keep working.
...
- Publish the two shortcuts as iCloud links and set `KlypstLinks.actionButtonShortcut` and `KlypstLinks.imageShortcut` (Shortcuts app → Share → Copy iCloud Link). Until then Help shows the manual steps.
